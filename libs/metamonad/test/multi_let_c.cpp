...
BOOST_AUTO_TEST_CASE(test_multi_let_c)
{
  using mpllibs::metamonad::multi_let_c;
  ...
  BOOST_MPL_ASSERT((
    is_same<
      int13,
      eval_syntax<multi_let_c<map<pair<x, syntax<int13> > >, x> >::type
    >
  ));

  // test_nothing_to_set
  BOOST_MPL_ASSERT((is_same<x, eval_syntax<multi_let_c<map<>, x> >::type>));
